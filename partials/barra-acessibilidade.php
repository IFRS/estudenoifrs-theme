<nav class="barra-acessibilidade">
    <ul class="barra-acessibilidade__menu">
        <li><a href="#inicio-conteudo" accesskey="1">Ir para o conteúdo [1]</a></li>
        <li><a href="#inicio-menu" accesskey="2">Ir para o menu [2]</a></li>
        <li><a href="#search-field" accesskey="3">Ir para a busca [3]</a></li>
        <li><a href="#inicio-rodape" accesskey="4">Ir para o rodapé [4]</a></li>
    </ul>

    <ul class="barra-acessibilidade__menu">
        <li><a href="<?php echo get_permalink(get_page_by_path('acessibilidade')); ?>">Acessibilidade</a></li>
        <li><a href="#site-map">Mapa do Site</a></li>
    </ul>
</nav>
